...
- This iteration grants **club-wide access**, including every team within the membership's club. Do not provision a user who should see only one team until team-scoped membership checks are implemented.
...
Deploy **one persistent Node server**, run migrations before starting, then `npm run build` and `npm start`. Serve behind HTTPS and restrict database-file access to the service account. SQLite is not suitable on ephemeral Vercel/serverless disks or across multiple machines. A managed PostgreSQL adapter and migration are needed for that hosting model. Database tracing is deliberately disabled: deploy the database as runtime storage, never as a bundled application asset.
