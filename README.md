...
Create teams in **Admin**, or import into an empty club before creating teams. **Home** summarises the active team, **My Team** shows player cards, **Players** retains section assignment/editing plus cross-team squad pooling, **Fixtures** retains all three match workflows, and **Selection** opens the pitch editor. **Player assessments** remains available from the context bar. **Registrations** (visible only to Northern Hockey Admins) shows Hockey England submission status across every team in the club.
...
- The **Northern Hockey Admin** role (`northern_hockey_admin`) sees every team in the club but is deliberately excluded from `canManage`/`canAdmin`: it does not gain ordinary workspace write access. It is the only role permitted to configure/sync England Hockey fixtures (`app/(dashboard)/fixtures/england-hockey-settings.tsx`, enforced server-side in `lib/repository.ts`'s `requireNorthernAdmin`), it views the cross-team **Registrations** dashboard (`team_submissions` table, `not_started`/`in_progress`/`submitted`), and — together with a team's own manager/coach/captain — it can pool a player into an additional team's roster without changing their home team (`player_team_memberships` table; `lib/repository.ts`'s `requireTeamManageAccess`). Club administrators assign it from **Admin → People & access → Manage roles**.
- A member may hold **more than one role** for the same club (e.g. a club administrator who is also a team captain) and switch which is active from a selector in the topbar, next to the team switcher. Switching is a real server-side permission change, not cosmetic: `lib/repository.ts`'s `listClubs` resolves exactly one **active** role per club (from `active_roles`, defaulting via `lib/users.ts`'s `ROLE_PRIORITY`/`defaultActiveRole` when unset) and every authorization check in the app keys off that one role — the member's other, unselected roles grant nothing until they switch to one of them. `club_memberships` and `membership_team_access` are keyed by `(user_id, club_id, role)` to allow this. Switching (`lib/active-role.ts`'s `switchActiveRole`, `PATCH /api/active-role`) is self-service and can only select a role the member already holds. Admins grant/revoke roles as a set of checkboxes at **Admin → People & access → Manage roles** (`lib/members.ts`'s `addMemberRole`/`removeMemberRole`); granting yourself an extra role is allowed (it can't exceed your existing admin access), but removing your own role, or a member's last remaining role, is blocked.
...
Fixture-specific availability, captaincy, fine-grained per-permission team roles, invitations and self-service password recovery are deferred. Multi-team player memberships now exist for squad pooling only (a player can be added to an additional team's roster); fixtures, lineups and formations still key off a player's single home team. Assessment attendance is never presented as confirmed match availability. No player prices, budgets or transfer mechanics are included.
...
Club administrators can change an existing member's role from **Admin → People & access → Manage roles** in the app; account creation and suspension are still operator-only. Operator SQL examples (use a private SQLite session against `DATABASE_PATH`; substitute actual IDs):
